---
id: sig-2026-04-21-plan-60-04-needed-an-auto-fix-for-installed-to-sou
type: signal
...
evidence:
  supporting:
    - 60-04 summary says installed runtime paths were being resolved against the repo root, recreating the known dogfooding false-positive pattern.
    - The fix mapped installed get-shit-done-reflect paths back to source get-shit-done paths and treated historical local-patch-only entries with no installed hash as stale.
  counter:
    - The correction landed in the same plan, and the golden fixture plus regression tests passed afterward.
confidence: high
confidence_basis: The summary marks this as an auto-fixed bug and describes both the misclassification and the repair.
triage: {}
remediation: {}
verification: {}
recurrence_of: ""
...
Plan 60-04 needed an auto-fix for installed-to-source path mapping to avoid misclassifying historical backup entries. Supporting evidence: 60-04 summary says installed runtime paths were being resolved against the repo root, recreating the known dogfooding false-positive pattern. The fix mapped installed get-shit-done-reflect paths back to source get-shit-done paths and treated historical local-patch-only entries with no installed hash as stale.

...
4. Fill ALL fields from the candidate data:
   ...
   - Provenance: runtime, model (from sensor output if available)
5. Set `lifecycle_state: detected` (all new signals start as detected)
6. Write body sections: What Happened (from summary + evidence.supporting), Context (from context object), Potential Cause (synthesizer's assessment)
7. Ensure directory exists: `mkdir -p ~/.gsd/knowledge/signals/{project}/`
8. Write the file

...
Extract from `agents/gsd-signal-collector.md` into a standalone sensor agent. The artifact sensor retains detection and classification logic from the signal-collector (Steps 1-4: load artifacts, load config, detect signals, classify signals) but does NOT filter traces, write to the KB, or rebuild the index -- ALL quality gating (trace filtering, dedup, rigor, caps) is the synthesizer's responsibility.
...
4. Remove Step 5 (filter traces -- trace filtering is the synthesizer's job per signal-detection.md Section 6), Step 6 (dedup check), Step 7 (cap check), Step 8 (write signals), Step 9 (rebuild index), Step 10 (report) from the execution flow
5. Add a NEW final step: "Return Results" that returns ALL signal candidates (including trace-severity) as structured JSON using delimited output format:
...
7. Keep severity auto-assignment (signal-detection.md Section 6) -- sensors classify, synthesizer validates. Return ALL severities including trace (the synthesizer handles trace filtering).
...
11. **Important:** Do NOT filter trace signals. Return ALL candidates regardless of severity. The synthesizer (gsd-signal-synthesizer) is the single enforcement point for trace non-persistence per signal-detection.md Section 6.
...
4. The execution flow should be a single step: "Return empty signal array with structured delimiters: `## SENSOR OUTPUT\n```json\n{ "sensor": "log", "phase": N, "signals": [] }\n```\n## END SENSOR OUTPUT`"
...
```
Verify artifact sensor has detection steps but NO write/index/filter steps:
```bash
...
grep -c "Filter Trace\|filter trace\|separate.*trace\|persistable" agents/gsd-artifact-sensor.md
# Should be 0 -- no trace filtering (synthesizer's job)
...
grep -c "SENSOR OUTPUT" agents/gsd-artifact-sensor.md
# Should be >= 1 -- structured delimiters present
```
  </verify>
  <done>
...
**Update `get-shit-done/workflows/signal.md`:**

Update the reference note that mentions `gsd-signal-collector` as a consumer of signal-detection.md. Change to: "gsd-artifact-sensor, gsd-signal-synthesizer, gsd-spike-runner" to reflect the new multi-sensor architecture.

**Test count update:** The existing tests may assert the feature manifest has 4 features (updated in Phase 31). Check `get-shit-done/bin/gsd-tools.test.js` for any feature count assertions and update from 4 to 5.
  </action>
  <verify>
```bash
# Validate JSON
...
<verification>
1. `agents/gsd-artifact-sensor.md` exists with detection logic but no KB write steps and no trace filtering
2. `agents/gsd-artifact-sensor.md` uses structured `## SENSOR OUTPUT` / `## END SENSOR OUTPUT` delimiters
3. `agents/gsd-log-sensor.md` exists as disabled stub with spike question and structured delimiters
4. `agents/gsd-signal-collector.md` has deprecation notice
5. `get-shit-done/feature-manifest.json` has signal_collection with 3 sensors (artifact enabled, git enabled, log disabled)
6. `get-shit-done/workflows/signal.md` references updated from gsd-signal-collector to new agents
7. `npm test` passes (feature count assertions updated to 5)
</verification>

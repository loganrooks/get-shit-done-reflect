...
Collect all successfully parsed sensor JSON arrays into a merged list.

Track: `TOTAL_CANDIDATES`, `SENSORS_COMPLETED`, `SENSORS_FAILED`, `SENSORS_FALLBACK` (used fallback extraction).
...
# Has JSON extraction with delimiter protocol
grep -c "SENSOR OUTPUT\|delimiter\|fallback" get-shit-done/workflows/collect-signals.md
# Should be >= 1
...
3. `get-shit-done/workflows/collect-signals.md` has JSON extraction logic with `## SENSOR OUTPUT` delimiter parsing and fallback
4. `get-shit-done/workflows/collect-signals.md` passes sensor output to synthesizer
5. `commands/gsd/collect-signals.md` updated for multi-sensor description
6. All 4 new agent files installed in `.claude/agents/`
7. Updated workflow and manifest installed in `.claude/`
8. Path prefix conversion (`~/` to `./`) verified in installed files
9. `npm test` passes
</verification>

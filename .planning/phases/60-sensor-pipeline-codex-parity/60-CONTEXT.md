...
- [assumed:reasoned] Subagent / sub-thread session files exist on both runtimes and should be deprioritized during triage. Claude Code writes separate JSONLs for subagent sessions (the spec calls these "mechanical, not conversational"). Codex equivalents have not been empirically enumerated — deferred to Phase 60 research (Q1 in the Open Questions section below).

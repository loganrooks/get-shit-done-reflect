...
**Important:** Read each template file first to understand its current structure, then add the Open Questions section in a location that makes sense with the existing content. Do not remove or modify existing sections.
  ...
  <verify>
Confirm each template file was actually modified with the Open Questions section:

```bash
# Verify project.md has Open Questions section
grep -A 5 '## Open Questions' get-shit-done/templates/project.md

# Verify context.md has Open Questions section
grep -A 5 '## Open Questions' get-shit-done/templates/context.md

# Verify research.md has Open Questions section with Genuine Gaps subsection
grep -A 10 '### Genuine Gaps' get-shit-done/templates/research.md
```

All three commands should return content, confirming the sections were added.
  </verify>
  <done>

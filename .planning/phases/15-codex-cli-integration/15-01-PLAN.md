...
4. **`generateCodexAgentsMd(targetDir, pathPrefix)`** -- Generates ~/.codex/AGENTS.md:
   ...
   - Content: GSD workflow awareness section (~2KB, well under 4KB limit). Include: available commands table (key commands: gsd-help, gsd-new-project, gsd-plan-phase, gsd-execute-phase, gsd-resume-work, gsd-pause-work, gsd-progress, gsd-signal), workflow conventions, KB location (`~/.gsd/knowledge/`)
   - **Codex capability limitations section (CODEX-06):** Content MUST explicitly document these Codex capability gaps:
     ...
     - Include a reference to the capability matrix: "For full runtime comparison, read the file at `{pathPrefix}/get-shit-done/references/capability-matrix.md`"
   - **`codex exec` non-interactive usage section (CODEX-07):** Include an explicit example showing scripted/CI usage:
     ```
...
- `generateCodexAgentsMd` function
  ...
  <done>
All 4 Codex adapter components exist in install.js. `convertClaudeToCodexSkill` is exported for testing. The functions follow the established patterns from OpenCode/Gemini adapters. AGENTS.md content explicitly documents Codex capability gaps (no Task tool, no hooks, no tool restrictions) with capability matrix reference, and includes codex exec usage examples.
  </done>
</task>
...
6. **`install(isGlobal, runtime)`** (line 1426):
   ...
   ```
   - **get-shit-done reference docs**: The existing `copyWithPathReplacement()` already handles all runtimes including Codex. It uses the standard (non-TOML, non-OpenCode) code path and correctly replaces `~/.claude/` with `~/.codex/` based on pathPrefix. No additional changes needed for reference doc copying.
...
- AGENTS.md documents Codex capability limitations (no Task tool, no hooks, no tool restrictions) with capability matrix reference
- AGENTS.md includes codex exec non-interactive usage examples for CI/scripted environments

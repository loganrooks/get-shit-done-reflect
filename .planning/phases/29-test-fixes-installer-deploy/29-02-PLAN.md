...
<task type="auto">
  <name>Task 0: Confirm stale state -- verify hash mismatch before install</name>
  <files>.claude/get-shit-done/bin/gsd-tools.js</files>
  <action>
  Before running the installer, prove the stale state exists by comparing hashes of source vs deployed binaries:

  ```bash
  echo "=== Source vs Deployed Hash Comparison (BEFORE install) ==="
  shasum get-shit-done/bin/gsd-tools.js \
        .claude/get-shit-done/bin/gsd-tools.js \
        ~/.claude/get-shit-done/bin/gsd-tools.js

  echo "=== Line Count Comparison (BEFORE install) ==="
  wc -l get-shit-done/bin/gsd-tools.js \
       .claude/get-shit-done/bin/gsd-tools.js \
       ~/.claude/get-shit-done/bin/gsd-tools.js
  ```

  **Expected:** Source hash differs from both deployed copies. Source should be ~5472 lines; deployed copies should be ~4597 lines (875-line gap). Both deployed copies should have the same hash as each other (both stale from the same prior install).

  **If all three hashes already match:** STOP. The installer has already been run and the gap is closed. Skip Tasks 1 and 2. Document this finding and proceed directly to verification.

  **If source differs from deployed (expected case):** Document the exact hashes and line counts as the "before" baseline. Proceed to Task 1.
  </action>
  <verify>
  Hash comparison output shows source hash differs from deployed hashes (proving the stale state).
  Line count comparison shows the ~875-line gap.
  Both values are documented as the "before" baseline for comparison after install.
  </verify>
  <done>Stale state is confirmed: source and deployed binaries have different hashes, proving the 875-line gap is real and the installer needs to run.</done>
</task>

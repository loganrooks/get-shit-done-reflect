...
must_haves:
  truths:
    ...
    - "pre-Phase-26 backlog items (without milestone field) parse correctly with milestone defaulting to null"
    - "promote and update on items without milestone field add it without corrupting other fields"
  artifacts:
...
<feature>
  ...
  <behavior>
    ...
    **TDD protocol: Write all ~14 tests below first (RED), then implement to pass all (GREEN). Do NOT write implementation code before all tests exist and fail.**

    **Feature tests (7 behaviors):**
    ...
    **Backward compatibility tests (3 tests -- catch "broke an old reader" before implementation):**

    8. Pre-Phase-26 items (no milestone line in file) parse correctly via backlog list
       - Manually create item file WITHOUT milestone field (simulating pre-Phase-26 item)
       - `backlog list --raw` -> item has `milestone: null` (defaulted, not error)

    9. backlog promote on item without milestone field preserves existing fields and adds milestone
       - Create item manually without milestone line
       - `backlog promote <id> --to REQ-01 --milestone v2.0` -> file has milestone: v2.0
       - All other fields (id, title, tags, status, promoted_to) intact

    10. backlog update --milestone on item without prior milestone field adds it
        - Create item manually without milestone line
        - `backlog update <id> --milestone v1.5` -> file has milestone: v1.5
        - No other fields corrupted
  </behavior>
  <implementation>
...
- ~14 new tests: 7 feature behaviors + 4 multi-case feature tests + 3 backward-compat tests
- Multi-status filter test
- All 143 existing tests still pass (zero regressions)
</verification>

<success_criteria>
- ~14 tests written in RED phase before any implementation code
...
- Pre-Phase-26 items (no milestone field) parse with null default
- Promote/update on items without milestone field add it without corrupting other fields

...
must_haves:
  truths:
    ...
    - "reflection-patterns.md documents counter-evidence seeking protocol with 3-counter-example bound using index metadata first"
    ...
    - "Section 2.2 includes secondary clustering fallback (same project + 3+ overlapping tags, any signal_type) for when primary clustering yields fewer than 5 patterns"
    - "Section 8 category taxonomy is declared authoritative and reconciled with agent spec categories"
  artifacts:
...
5. **Update Section 2.2 (Clustering Criteria)** to add a secondary clustering fallback. The current primary algorithm groups by `signal_type` + 2+ overlapping tags. This causes `signal_type` fragmentation -- signals about the same issue typed as `deviation`, `struggle`, and `config-mismatch` won't cluster together. Add:

   **Secondary clustering fallback (new):** When primary clustering yields fewer than 5 qualifying patterns, apply a relaxed mode:
   - Same project + 3+ overlapping tags + any signal_type
   - Still requires positive/negative to cluster separately
   - Secondary clusters are marked as "cross-type" in output and get a 0.8x score multiplier (slightly penalized for weaker signal_type coherence)
   - This ensures the reflector can still find thematic patterns even when signal_type is fragmented across the corpus

   Preserve Section 2.3 (Pattern Output Format) and update it to include "Weighted Score" alongside Confidence.
...
8. **Reconcile Section 8 (Category Taxonomy):** The current taxonomy in Section 8 uses `tooling|architecture|testing|workflow|external|environment` but the reflector agent spec uses `architecture|workflow|tooling|testing|debugging|performance|other`. Declare Section 8 as the **authoritative** taxonomy. Add a note: "This taxonomy is authoritative. The reflector agent and lesson templates use these categories. Legacy categories (`debugging`, `performance`, `other`) from earlier agent spec versions map to: `debugging` -> `testing`, `performance` -> `architecture`, `other` -> `workflow`." This resolves a pre-existing discrepancy between the reference doc and agent spec.
...
6. `grep "cross-type\|secondary clustering\|3+ overlapping tags" get-shit-done/references/reflection-patterns.md` returns matches (fallback clustering)
7. `grep "Index-first\|index metadata" get-shit-done/references/reflection-patterns.md` returns match (counter-evidence optimization)
8. `grep "authoritative" get-shit-done/references/reflection-patterns.md` Section 8 returns match (taxonomy reconciliation)
</verification>

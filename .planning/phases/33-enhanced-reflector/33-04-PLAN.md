...
3. Verify the output includes:
   a. A Lifecycle Dashboard at the top showing signal counts by state (all 50 should show as Untriaged/detected since no triage has been performed yet)
   b. Patterns Detected with confidence-weighted scores (not just raw occurrence counts)
   c. Triage Proposals for detected signal clusters with recommended decisions (address/dismiss/defer/investigate)
   d. Lesson Candidates (should find 5+ from the 7 identified clusters)
...
5. Check that the reflector does NOT modify any SIG-format signal files (legacy read-only)
6. Check that approved lessons include evidence_snapshots in their frontmatter
  </verify>
  <done>

...
must_haves:
  truths:
    ...
    - "Reflect workflow outputs a lifecycle dashboard showing signal counts by state, with SIG-format signals in a separate Legacy row"
    ...
    - "Reflect workflow enforces per-run triage cap of 10 signals to bound first-run bulk operations"
    - "YOLO triage auto-approve is limited to address and dismiss decisions only; defer and investigate always prompt"
    - "Reflect output includes Phase 34 dependency note explaining triaged signals remain at triaged until Signal-Plan Linkage ships"
  artifacts:
...
a. Reads the KB index and counts signals by lifecycle_state. **SIG-format signals (ID starts with `SIG-`) are counted separately in a "Legacy (read-only)" row** rather than inflating the "Untriaged" count. This prevents 15 permanently-untriageable signals from creating noise as the project matures:
   - `detected` (standard-format only, or missing lifecycle_state for non-SIG signals) -> "Untriaged"
   - `triaged` -> "Triaged"
   - `remediated` -> "Remediated"
   - `verified` -> "Verified"
   - `invalidated` -> "Invalidated"
   - SIG-format (ID starts with `SIG-`) -> "Legacy (read-only)"
...
b. **YOLO mode (triage has higher blast radius than lessons -- modifies existing files, not just creating new ones):**
   - Auto-approve `address` and `dismiss` decisions ONLY (these have clear intent)
   - Present `defer` decisions for user confirmation (deferral is a judgment call)
   ...
   - **Design rationale:** Unlike lesson auto-approve (creates new files), triage auto-approve modifies existing signal files. Limiting auto-approve to `address` and `dismiss` bounds the blast radius of YOLO mode for triage operations.

c. **Per-run triage cap:** Enforce a maximum of 10 signals triaged per reflect run. If more than 10 signals would be triaged across all approved proposals, present the highest-priority proposals first and queue the remainder with a note: "Triage cap reached (10 signals). Run /gsd:reflect again to continue triaging remaining clusters." This prevents the first-run scenario where 30+ signal files are modified in a single session, producing an unwieldy git commit and risking context budget exhaustion in the reflector agent.

d. **When triage is approved:** The workflow instructs the reflector agent to write triage fields to each signal in the cluster. The workflow itself does NOT write files -- the reflector does.

e. **Phase 33 triage constraint reminder:** Include a note that when triaging critical signals without lifecycle_state, evidence.supporting MUST be added first. Reference knowledge-store.md Section 4.2.
...
**3a. Phase 34 Dependency Note (new)**

Add a note at the end of the remediation suggestions section:
```markdown
...
This sets user expectations that triage is a partial lifecycle step, not the complete loop.
...
6. `grep "Legacy (read-only)" get-shit-done/workflows/reflect.md` returns match (SIG-format dashboard row)
7. `grep "triage cap\|10 signals" get-shit-done/workflows/reflect.md` returns match (per-run cap)
8. `grep "Phase 34" get-shit-done/workflows/reflect.md` returns match (temporal gap note)
9. `grep "address.*dismiss.*ONLY\|blast radius" get-shit-done/workflows/reflect.md` returns match (YOLO scope)
</verification>

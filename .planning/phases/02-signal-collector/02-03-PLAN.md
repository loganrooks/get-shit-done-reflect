...
- File contains `signal-detection.md` reference (confirms severity rules and frustration patterns are sourced)
- File contains frustration detection logic (e.g., mentions frustration patterns, conversation scanning, or SGNL-06)
- Includes preview-before-save step
- File contains `related_signals` or dedup logic (confirms deduplication check)
- File contains cap check logic (confirms SGNL-09 enforcement)
- File contains `kb-rebuild-index` reference (confirms index rebuild)

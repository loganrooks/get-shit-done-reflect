...
must_haves:
  truths:
    - "All vitest tests pass with 0 failures (npx vitest run exits cleanly) AND smoke tests pass if claude CLI is available (or user explicitly approved vitest-only validation)"
  ...
  key_links:
    - from: "npx vitest run + smoke tests"
      to: "git tag v1.12.2-pre-sync"
      via: "vitest + smoke tests (or user-approved vitest-only) must pass before tag is created"
...
1. **Verify clean state (unit + integration + smoke):**
   - Run `npx vitest run` to confirm all unit/integration tests pass
   - **Smoke test validation (user requirement):** The user's phase context requires "test suite passing + smoke test passing" before creating the snapshot. Run the smoke tests:
     ```bash
     # Check if claude CLI is available (required for smoke tests)
     which claude 2>/dev/null
     ```
     - **If `claude` CLI IS available:** Run `tests/smoke/run-smoke.sh` (or the project's smoke test entry point). Smoke tests make real API calls and take minutes to complete. Both vitest AND smoke tests must pass before proceeding to step 2.
     - **If `claude` CLI is NOT available:** STOP and present a checkpoint to the user:

       > **CHECKPOINT: Smoke tests require `claude` CLI**
       >
       > Your phase context requires smoke tests to pass before creating the pre-merge snapshot, but smoke tests require:
       > - The `claude` CLI installed and on PATH
       > - Active API keys configured
       > - Real API calls (costs money, takes minutes)
       >
       > Options:
       > - **(A) Set up claude CLI now** -- install/configure it, then re-run this task
       > - **(B) Proceed with vitest-only validation** -- unit + integration tests passed; smoke tests deferred to post-Phase-8-merge validation
       >
       > The research recommended option (B) as sufficient for the pre-merge snapshot, with full smoke validation after Phase 8 merge.
...
<verification>
1. `npx vitest run` -- all unit/integration tests pass, 0 failures
1b. Smoke tests pass (if `claude` CLI available) OR user approved vitest-only validation at checkpoint
...
<success_criteria>
- All vitest tests pass with 0 failures AND smoke tests pass (or user approved vitest-only at checkpoint)

...
- Update persistence rules to include `minor`:
  ...
  - trace: logged in collection report only, NOT written to KB
...
- `grep "1.2.0" get-shit-done/references/signal-detection.md` returns version update
  ...
  <done>
signal-detection.md uses four-tier severity model, documents signal_category field, includes positive signal detection rules (baseline/improvement/good-pattern), and defines epistemic gap detection -- all consistent with the schema in knowledge-store.md.
  </done>
</task>
...
- Update the pseudocode to match the new severity names:
  ```

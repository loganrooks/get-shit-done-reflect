...
must_haves:
  truths:
    ...
    - "Existing date-slug format signals (31 of 46) load via extractFrontmatter without errors and pass schema validation -- missing lifecycle fields default gracefully as recommended (not required). 15 legacy SIG-format signals are readable via extractFrontmatter but are not subject to schema validation (they predate the standard schema)."
    - "kb-rebuild-index.sh extracts lifecycle_state and displays it in the Lifecycle column of the signals table"
    - "Tests cover signal schema validation for all four severity tiers, backward compatibility with date-slug format signals, and conditional evidence requirements"
    - "FORK-DIVERGENCES.md records the FRONTMATTER_SCHEMAS signal schema addition"
  artifacts:
    ...
    - path: "get-shit-done/bin/gsd-tools.test.js"
      ...
      contains: "signal"
    - path: "agents/kb-rebuild-index.sh"
      provides: "Index with lifecycle_state column"
      contains: "Lifecycle"
    - path: ".planning/FORK-DIVERGENCES.md"
      provides: "Fork divergence tracking for gsd-tools.js signal schema"
      contains: "signal"
  key_links:
    ...
    - from: "agents/kb-rebuild-index.sh"
...
<objective>
Extend FRONTMATTER_SCHEMAS in gsd-tools.js with signal schema validation (including conditional requirements by severity), add comprehensive tests for backward compatibility and tiered validation, update kb-rebuild-index.sh to extract the lifecycle_state column, and track the fork divergence.
...
Output: Extended gsd-tools.js validation, test coverage, updated index script, and fork divergence entry.
</objective>
...
**Step 3: Track fork divergence.**

Add an entry to `.planning/FORK-DIVERGENCES.md` recording the FRONTMATTER_SCHEMAS signal schema addition with merge stance `keep-fork -- upstream has no signal schema`. Include the phase reference (31) and the specific change (added `signal` key to FRONTMATTER_SCHEMAS, extended cmdFrontmatterValidate with conditional/recommended support).
  </action>
  <verify>
...
- `grep "signal" .planning/FORK-DIVERGENCES.md` returns the divergence entry
  </verify>
  <done>
FRONTMATTER_SCHEMAS includes signal schema with tiered validation. cmdFrontmatterValidate handles required, conditional, and recommended fields. Existing plan/summary/verification schemas are unaffected. Fork divergence is tracked in FORK-DIVERGENCES.md.
  </done>
</task>

<task type="auto">
  <name>Task 2: Add signal validation tests and update kb-rebuild-index.sh</name>
  <files>get-shit-done/bin/gsd-tools.test.js, agents/kb-rebuild-index.sh</files>
  <action>
...
6. **Backward compatibility: date-slug format signal** -- Create a signal file matching the format of the 31 date-slug signals that represent current practice (id: sig-2026-02-28-example, type: signal, project, tags, created, severity: notable, signal_type: deviation, phase, plan, polarity, source, occurrence_count, related_signals, runtime, model -- NO lifecycle_state, NO evidence, NO confidence). Assert valid: true (lifecycle_state is recommended, not required). Note: The 15 legacy SIG-format signals (SIG-260222-*, SIG-260223-*) use a completely different schema (type: positive-pattern/architecture/deviation, date: instead of created:, no signal_type field) and are NOT subject to this schema validation. Schema validation is opt-in -- agents call `frontmatter validate --schema signal` only on NEW signals they create.
...
**Part B: Update agents/kb-rebuild-index.sh (npm source per CLAUDE.md dual-directory rule -- do NOT edit ~/.gsd/bin/ or .claude/agents/ directly).**
...
After updating the npm source, run `node bin/install.js --local` to propagate to .claude/agents/ and then to ~/.gsd/bin/. Then run the index rebuild to verify: `bash ~/.gsd/bin/kb-rebuild-index.sh` and check the output has the Lifecycle column.
  </action>
  <verify>
- `npm test` passes (all existing tests + new signal validation tests)
- `grep "lifecycle_state" agents/kb-rebuild-index.sh` returns the extraction line
- `grep "Lifecycle" agents/kb-rebuild-index.sh` returns the column header
- Run `node bin/install.js --local` to propagate changes, then `bash ~/.gsd/bin/kb-rebuild-index.sh` and verify `grep "Lifecycle" ~/.gsd/knowledge/index.md` shows the column
- Run the backward compatibility test: validate an existing date-slug format signal file from ~/.gsd/knowledge/signals/get-shit-done-reflect/ using `node get-shit-done/bin/gsd-tools.js frontmatter validate <path> --schema signal` -- should return valid: true
  </verify>
  <done>
Signal validation tests cover all four severity tiers, backward compatibility with date-slug format signals (with note that SIG-format legacy signals are exempt from schema validation), and conditional evidence requirements. kb-rebuild-index.sh extracts lifecycle_state into a Lifecycle column with "detected" default for existing signals.
  </done>
</task>
...
- [ ] At least one existing date-slug format signal file validates successfully with the new schema
- [ ] kb-rebuild-index.sh produces index with Lifecycle column
- [ ] Existing signals show "detected" in the Lifecycle column
- [ ] FORK-DIVERGENCES.md records the signal schema addition with merge stance
- [ ] Run `node bin/install.js --local` to sync all source changes to .claude/ directory
</verification>

<success_criteria>
The signal schema is machine-enforceable: gsd-tools.js validates signal frontmatter with tiered rigor (critical signals require evidence), existing date-slug format signals pass validation, legacy SIG-format signals are acknowledged as exempt, the knowledge base index includes lifecycle state, and the fork divergence is tracked. All tests pass.
</success_criteria>

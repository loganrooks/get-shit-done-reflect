...
<task type="auto">
  <name>Task 1: Write failing tests for log-migration and auto-detect commands</name>
  <files>
    get-shit-done/bin/gsd-tools.test.js
  </files>
  <action>
**TDD RED phase:** Write all tests first. They WILL fail because `manifest log-migration` and `manifest auto-detect` do not exist yet. That is expected and correct.

**Part A: Add `describe('manifest log-migration', ...)` block with these tests:**
...
**Part B: Add `describe('manifest auto-detect', ...)` block with these tests:**
...
**Part C: Run test suite — confirm 14 new tests fail, existing tests pass**

Run the full test suite. The 14 new tests MUST fail (commands don't exist yet). All existing tests (including the 9 apply-migration tests from Plan 01) MUST still pass. This confirms the red phase is complete.
  </action>
  <verify>
1. Run: `node --test get-shit-done/bin/gsd-tools.test.js 2>&1 | tail -10` -- 14 new tests fail, all existing tests pass
2. `grep -c 'log-migration' get-shit-done/bin/gsd-tools.test.js` returns 8+ (describe + tests)
3. `grep -c 'auto-detect' get-shit-done/bin/gsd-tools.test.js` returns 8+ (describe + tests)
  </verify>
  <done>
14 tests written and confirmed failing (7 log-migration + 7 auto-detect). Each test specifies exact expected behavior. Existing test suite (including Plan 01's apply-migration tests) still passes. TDD red phase complete.
  </done>
</task>

<task type="auto">
  <name>Task 2: Implement log-migration and auto-detect commands to make all tests pass</name>
  <files>
    get-shit-done/bin/gsd-tools.js
  </files>
  <action>
**TDD GREEN phase:** Implement the minimum code to make all 14 tests pass.
...
**Part C: Add cmdManifestAutoDetect function**
...
**Part D: Wire both commands into CLI router**
...
Update the error message to include log-migration and auto-detect in available subcommands.

**Part E: Run tests — all 14 must now pass**

Run the full test suite. All 14 new tests (7 log-migration + 7 auto-detect) must pass. All existing tests (including Plan 01's apply-migration tests) must still pass. If any test fails, fix the implementation (not the test) until green.
  </action>
  <verify>
1. Run `node --test get-shit-done/bin/gsd-tools.test.js` -- ALL tests pass (0 failures)
2. Run from project root: `node get-shit-done/bin/gsd-tools.js manifest auto-detect devops --raw` -- should detect github-actions (this project has .github/workflows)
3. Run from project root: `node get-shit-done/bin/gsd-tools.js manifest auto-detect release --raw` -- should detect package.json
4. Run from project root: `node get-shit-done/bin/gsd-tools.js manifest auto-detect health_check --raw` -- should show `detected: {}`
5. Run in temp dir: `node get-shit-done/bin/gsd-tools.js manifest log-migration --from 1.12.0 --to 1.15.0 --changes '[{"type":"feature_added","config_key":"release","fields_added":["version_file","changelog"]}]' --raw` -- should output `{"logged":true,...}`
  </verify>
  <done>
TDD green phase complete. All 14 tests pass. `manifest log-migration` creates or appends to migration-log.md with correctly formatted entries for all change types. `manifest auto-detect` runs filesystem and git checks from manifest auto_detect rules. No regressions in existing tests.
  </done>
</task>

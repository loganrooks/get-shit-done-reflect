...
must_haves:
  truths:
    ...
    - "A self-test validates the real shipped feature-manifest.json has correct structure (3 features, all fields typed)"
    - "A defaults alignment test catches drift between manifest defaults and loadConfig() hardcoded defaults"
  artifacts:
...
**Test suite: `manifest self-test (real manifest)`** (add `describe('manifest self-test', ...)`)

These tests validate the REAL shipped `feature-manifest.json` (not synthetic fixtures), catching structural issues and default drift that fixture-based tests cannot detect.

16. **real manifest is valid JSON with expected structure** - Load the actual `get-shit-done/feature-manifest.json` from the repo root (NOT from a temp dir). Assert: parses as valid JSON, has `manifest_version` (integer), has `features` object, contains exactly 3 features (`health_check`, `devops`, `release`). For each feature, assert it has `scope`, `introduced`, `config_key`, `schema` (non-empty object), and `init_prompts` (array). For each field in each schema, assert it has `type`, `default`, and `description`.

17. **real manifest defaults match loadConfig() defaults** - This is the **Pitfall 1 drift detector**. Load the real `feature-manifest.json`. For each feature where `scope === "project"`, extract the `config_key` and all field defaults from the schema. Then run `node get-shit-done/bin/gsd-tools.js init plan-phase 1 --include state` against a temp project that has an EMPTY config.json (only `{}`) and read the effective config via `loadConfig()` — OR — directly read the `loadConfig()` function's hardcoded defaults from gsd-tools.js source and compare. The simpler approach: create a temp project with `config.json = {}`, run `manifest diff-config`, and verify every `missing_fields` entry's `default` value matches the hardcoded default in `loadConfig()`. At minimum, assert these critical defaults align:
...
If any default diverges, the test MUST fail with a message identifying which field drifted — e.g., `"Default drift: health_check.frequency — manifest says 'milestone-only', loadConfig says 'on-resume'"`.

**Why these tests matter:** All other tests use synthetic fixtures, so they'd pass even if the real manifest had typos or drifted defaults. These two tests are the permanent safety net against Pitfall 1 (default divergence) and structural regressions.
  </action>
  <verify>
Run `node --test get-shit-done/bin/gsd-tools.test.js 2>&1 | tail -5` and verify all tests pass including the new manifest tests. No existing tests should be broken.
  ...
  <done>All manifest command tests pass. Tests cover: missing features, missing fields, type mismatches, enum mismatches, unknown fields as warnings (not errors), validation pass/fail, feature counting, prompt retrieval, error cases, real manifest structural validation, and defaults alignment drift detection.</done>
</task>
...
- New manifest tests (17+ test cases) all pass
- Real manifest self-test validates structure of the shipped feature-manifest.json
- Defaults alignment test catches any drift between manifest and loadConfig() defaults

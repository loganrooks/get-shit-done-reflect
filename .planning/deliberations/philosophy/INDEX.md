# Philosophical Deliberations Index

**Created:** 2026-03-03
**Purpose:** Epistemological foundations for GSD Reflect's self-improvement system. These documents are citable in requirements, design decisions, and signal interpretation via the `philosophy:` motivation type.

## Citation Format

In requirements and design documents, cite as:
```
*Motivation:* `philosophy: <framework>/<principle>` — <explanation>
```

Example:
```
*Motivation:* `philosophy: falsificationism/severe-tests` — passive verification-by-absence is a weak test
```

## Frameworks

| Document | Tradition | Core Question for GSD Reflect |
|----------|-----------|-------------------------------|
| [falsificationism.md](falsificationism.md) | Critical Rationalism (Popper) | How do we test whether our improvements actually work? |
| [pragmatism.md](pragmatism.md) | American Pragmatism (Peirce, Dewey) | When does a lesson stop being warranted? |
| [lakatos.md](lakatos.md) | Post-Popperian (Lakatos) | Is our improvement programme progressive or degenerating? |
| [bayesian-epistemology.md](bayesian-epistemology.md) | Formal Epistemology (Bayes, Jeffrey) | How should confidence in lessons update with evidence? |
| [cybernetics.md](cybernetics.md) | Systems Theory (Wiener, Ashby, von Foerster) | What happens when the system observes itself? |
| [dialectics.md](dialectics.md) | German Idealism / Critical Theory (Hegel, Marx, Adorno) | How do improvements create new contradictions? |
| [toulmin.md](toulmin.md) | Argumentation Theory (Toulmin) | What makes an argument well-formed, and how do we evaluate competing claims? |
| [habermas.md](habermas.md) | Discourse Ethics (Habermas) | Under what conditions is automated deliberation legitimate? |
| [gadamer.md](gadamer.md) | Philosophical Hermeneutics (Gadamer) | How do we understand across contexts without losing situatedness? |
| [aristotle.md](aristotle.md) | Practical Philosophy (Aristotle) | What are the limits of formalizing good judgment? |
| [praxis-meta.md](praxis-meta.md) | Cross-cutting (meta-methodology) | How does philosophical thinking become technical design? |
| [contradictions-in-v17-requirements.md](contradictions-in-v17-requirements.md) | Multi-framework analysis (Adorno, Lakatos, Dewey) | What contradictions in the requirements resist resolution? |

## All Citable Principles

### Falsificationism
- `falsificationism/falsifiable-predictions`
- `falsificationism/severe-tests`
- `falsificationism/corroboration-not-confirmation`
- `falsificationism/asymmetric-evidence`
- `falsificationism/theory-laden-observation`
- `falsificationism/bold-over-timid`
- `falsificationism/verisimilitude-through-revision`
- `falsificationism/auxiliary-assumptions`

### Pragmatism
- `pragmatism/warranted-assertibility`
- `pragmatism/fallibilism`
- `pragmatism/abductive-inference`
- `pragmatism/inquiry-cycle`
- `pragmatism/cash-value`
- `pragmatism/community-of-inquiry`
- `pragmatism/do-not-block-inquiry`
- `pragmatism/knowledge-through-use`

### Lakatos
- `lakatos/progressive-improvement`
- `lakatos/hard-core-protection`
- `lakatos/novel-prediction`
- `lakatos/programme-trajectory`
- `lakatos/no-crucial-test`
- `lakatos/positive-heuristic`
- `lakatos/rival-programme`
- `lakatos/degeneration-detection`

### Bayesian Epistemology
- `bayesian/belief-updating`
- `bayesian/prior-sensitivity`
- `bayesian/evidence-independence`
- `bayesian/old-evidence`
- `bayesian/diagnosticity`
- `bayesian/directional-updating`
- `bayesian/calibration`
- `bayesian/coherence`
- `bayesian/convergence`

### Cybernetics
- `cybernetics/requisite-variety`
- `cybernetics/observer-effect`
- `cybernetics/autopoietic-categories`
- `cybernetics/second-order-observation`
- `cybernetics/regime-change`
- `cybernetics/structural-coupling`
- `cybernetics/error-as-information`
- `cybernetics/viable-system`
- `cybernetics/degenerating-spiral`

### Dialectics
- `dialectics/aufhebung`
- `dialectics/negation-of-negation`
- `dialectics/negative-dialectics`
- `dialectics/praxis`
- `dialectics/concrete-universal`
- `dialectics/internal-contradiction`
- `dialectics/master-slave`
- `dialectics/base-superstructure`

### Toulmin
- `toulmin/explicit-warrants`
- `toulmin/rebuttal-conditions`
- `toulmin/field-dependence`
- `toulmin/qualifier-precision`
- `toulmin/warrant-comparison`
- `toulmin/graduated-formality`
- `toulmin/backing-depth`
- `toulmin/presumption-transparency`

### Habermas
- `habermas/communicative-vs-strategic`
- `habermas/validity-claims`
- `habermas/ideal-speech`
- `habermas/discourse-ethics`
- `habermas/colonization`
- `habermas/informed-consent`
- `habermas/lifeworld-system`
- `habermas/monological-limitation`

### Gadamer
- `gadamer/hermeneutic-circle`
- `gadamer/prejudice-as-condition`
- `gadamer/fusion-of-horizons`
- `gadamer/effective-historical-consciousness`
- `gadamer/question-and-answer`
- `gadamer/application`
- `gadamer/distanciation`
- `gadamer/horizon-as-limit`

### Aristotle
- `aristotle/phronesis`
- `aristotle/euboulia`
- `aristotle/deliberative-syllogism`
- `aristotle/mesotes`
- `aristotle/poiesis-praxis`
- `aristotle/hexis`
- `aristotle/techne-phronesis-distinction`
- `aristotle/formalization-limits`

### Praxis (Meta)
- `praxis/theory-practice-unity`
- `praxis/generative-deviation`
- `praxis/epistemic-grounding`
- `praxis/spiral-not-pipeline`

### Contradictions (Applied Analysis)
- `contradictions/structural-not-bugs`
- `contradictions/user-irreducibility`
- `contradictions/health-is-instrument`
- `contradictions/patience-with-trajectory`
- `contradictions/measure-with-humility`

---
*Index generated 2026-03-03. Updated after Toulmin, Habermas, Gadamer, Aristotle additions.*

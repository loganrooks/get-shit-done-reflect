...
function isValidConfigKey(keyPath) {
  ...
  // Fork: allow health_check.<sub-key> for fork health probe config
...
/**
 * Fork wrapper for config-get: returns {key, value, found} envelope for graceful absent-key
 * handling. Used by the fork router and fork tests that expect this response shape.
 * Upstream's cmdConfigGet returns the raw value directly; this wraps it consistently.
 */
function cmdConfigGetGraceful(cwd, keyPath, raw) {
  if (!keyPath) {

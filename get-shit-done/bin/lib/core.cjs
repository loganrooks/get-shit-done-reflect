...
function resolveModelInternal(cwd, agentType) {
  // Fork: normalize gsdr- prefix to gsd- for model lookup
  ...
  // Support both gsd- and gsdr- keys in model_overrides.
...
module.exports.loadManifest = function loadManifest(cwd) {
  ...
  const globalPath = path.join(os.homedir(), '.claude', 'get-shit-done', 'feature-manifest.json');
...
/**
 * Atomic JSON write — delegates to atomicWriteFileSync for consistent atomic semantics.
 * Replaces the fork's old separate tmp/rename implementation.
 */

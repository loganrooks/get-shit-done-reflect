...
<objective>
Create a manual signal entry in the knowledge base capturing an observation from the current conversation. apply_patch a signal file to the project's KB signals directory and rebuild the index.
</objective>
...
1. read_file index at `{KB_DIR}/index.md` (if exists)
...
if [ -d ".planning/knowledge" ]; then
  ...
  KB_REBUILD="bash get-shit-done-reflect/bin/kb-rebuild-index.sh"
else
...
## Step 7: apply_patch Signal File

apply_patch to `{KB_DIR}/signals/{project}/{YYYY-MM-DD}-{slug}.md` using schema above. Create parent directories with `mkdir -p`. Include all frontmatter fields and body sections (What Happened, Context, Potential Cause).
...
- apply_patch `detected_by` and `written_by` as separate signature objects even when the values overlap.
...
- Source always "manual" (vs "auto" from gsdr-signal-collector)
